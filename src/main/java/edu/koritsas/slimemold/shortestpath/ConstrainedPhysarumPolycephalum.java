...
import edu.koritsas.slimemold.shapefile.Constraint;
 ...
 */
public abstract class ConstrainedPhysarumPolycephalum {
    ...
    protected HashMap<Edge,Double> mWeights;
    ...
    private double[] lagrangeMultipliers = new double[mConstraints.size()];
    private int lagrangeCounter=0;


    public ConstrainedPhysarumPolycephalum(Graph graph, Node sourceNode, Node sinkNode,double absoluteThreshold,double relativeThreshold, int numberOfIterations){
        this.graph=graph;
        this.sourceNode=sourceNode;
        this.sinkNode=sinkNode;
    ...
    private boolean solutionViolatesConstraints(){


    return mConstraints.stream().anyMatch(Constraint::isViolated);
    }
